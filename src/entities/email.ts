...
const isLocalPartValid: IsLocalPartValid = email =>
  isLocalPartWithinMinSize(email) && isLocalPartWithinMaxSize(email);

type IsLocalPartWithinMinSize = (email: string) => boolean;
const isLocalPartWithinMinSize: IsLocalPartWithinMinSize = email =>
  !(email.split('@')[0].length === 0);

type IsLocalPartWithinMaxSize = (email: string) => boolean;
const isLocalPartWithinMaxSize: IsLocalPartWithinMaxSize = email =>
...
const isDomainPartWithMaxSize: IsDomainPartWithMaxSize = email =>
